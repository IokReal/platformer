...
class Camera:
    ...
    def apply(self, obj):
        obj.rect.x += self.dx
        # вычислим координату клeтки, если она уехала влево за границу экрана
        if obj.rect.x < -obj.rect.width:
            obj.rect.x += (self.field_size[0] + 1) * obj.rect.width
        # вычислим координату клeтки, если она уехала вправо за границу экрана
...
class Spoody(pygame.sprite.Sprite):
    image = load_image("spoody.png", colorkey=-1)
    def __init__(self, x, *group):
        super().__init__(*group)
        self.rect = self.image.get_rect()
        self.image = Spoody.image
        self.rect.x = 550
        self.rect.y = 465
...
pleer.earth = earth
total_time = 0
spoodies = [random.randint(0, 1500) for i in range(3)]
for spoody in spoodies:
    Spoody(spoody, all_sprites)
